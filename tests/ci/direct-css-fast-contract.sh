...
mkdir -p "$fixture_root/css" "$fixture_root/js" "$fixture_root/inc" "$fixture_root/tests/ci" "$fixture_root/.github/workflows"
...
classify() {
  ...
  (cd "$fixture_root" && "$classifier" "$event_name" "$base" "$head") | sed -n 's/^ci_profile=//p'
}
...
assert_profile() {
  ...
  local actual
  actual=$(classify "$event_name" "$base" "$head")
...
for changed_path in inc/runtime.php js/admin.js tests/ci/fixture.sh .github/workflows/ci.yml readme.txt; do
  reset_fixture
