...
$key = 'wcos_bulk_return_local_ui_fixture'; $manifest = get_option($key, array());
if (!is_array($manifest) || empty($manifest)) { echo "BULK_UI_ALREADY_CLEAN\n"; exit(0); }
global $wpdb;
...
if (!$orders_absent || wc_get_product($manifest['product_id']) || false !== get_option($key, false)) { fwrite(STDERR, "BULK_UI_CLEANUP_INCOMPLETE\n"); exit(2); }
echo "BULK_UI_CLEANUP_OK orders=absent product=absent manifest=absent\n";
