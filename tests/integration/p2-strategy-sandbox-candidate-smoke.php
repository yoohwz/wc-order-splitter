...
$candidate_admin_id = wp_insert_user(array(
	'user_login' => 'wcos_sandbox_ui4_' . wp_generate_password(8, false),
	'user_pass' => wp_generate_password(24, true),
	'user_email' => 'wcos-sandbox-ui4-' . wp_generate_uuid4() . '@example.test',
	'role' => 'administrator',
));
wcos_p2_adapter_assert(!is_wp_error($candidate_admin_id), 'Unable to create sandbox UI4 administrator.');
...
try {
	...
	$category_keep_term = wp_insert_term('WCOS UI4 Keep ' . $suffix, 'product_cat');
	$category_move_term = wp_insert_term('WCOS UI4 Move ' . $suffix, 'product_cat');
	wcos_p2_adapter_assert(!is_wp_error($category_keep_term) && !is_wp_error($category_move_term), 'Unable to create UI4 Category terms.');
	$category_keep = wcos_p2_adapter_product('WCOS UI4 Category Keep', '12.00', 30);
	$category_move = wcos_p2_adapter_product('WCOS UI4 Category Move', '8.00', 40);
	...
	wcos_p2_adapter_assert('completed' === $category_result['status'] && 1 === count($category_result['children']), 'UI4 Category Split did not create exactly one child.');
	$category_child_id = absint($category_result['children'][0]['id']);
	$category_source = wc_get_order($category_order_id);
	wcos_p2_adapter_assert($category_source->get_item($category_keep_item) instanceof WC_Order_Item_Product, 'UI4 Category Split lost the retained source bucket.');
	wcos_p2_adapter_assert(!$category_source->get_item($category_move_item), 'UI4 Category Split ignored frozen Review classification.');
	wcos_p2_adapter_assert($category_keep_stock == wc_get_product($category_keep->get_id())->get_stock_quantity(), 'UI4 Category Split changed retained-product stock.');
	wcos_p2_adapter_assert($category_move_stock == wc_get_product($category_move->get_id())->get_stock_quantity(), 'UI4 Category Split changed moved-product stock.');
	$category_journal = WCOS_Operation_Journal::get($category_source, $category_operation_id);
	wcos_p2_adapter_assert(is_array($category_journal) && 'completed' === $category_journal['status'], 'UI4 Category journal did not complete.');
	wcos_p2_adapter_assert(WCOS_Split_Strategy_Gates::CATEGORY === $category_journal['context']['strategy_authority']['strategy'], 'UI4 Category journal lost semantic strategy authority.');
	...
	wcos_p2_adapter_assert(1 === count($category_retry['children']) && $category_child_id === absint($category_retry['children'][0]['id']), 'UI4 Category durable retry returned a different child.');
	wcos_p2_adapter_assert(1 === count(wcos_p2_adapter_children($category_order_id, $category_operation_id)), 'UI4 Category durable retry created duplicate children.');

	/* Stock status: enabled Review -> Confirm -> Execute uses frozen volatile status. */
	$stock_keep = wcos_p2_adapter_product('WCOS UI4 Stock Keep', '11.00');
	$stock_move = wcos_p2_adapter_product('WCOS UI4 Stock Move', '6.00');
	...
	wcos_p2_adapter_assert('completed' === $stock_result['status'] && 1 === count($stock_result['children']), 'UI4 Stock-status Split did not create exactly one child.');
	$stock_source = wc_get_order($stock_order_id);
	wcos_p2_adapter_assert($stock_source->get_item($stock_keep_item) instanceof WC_Order_Item_Product, 'UI4 Stock-status Split lost selected source bucket.');
	wcos_p2_adapter_assert(!$stock_source->get_item($stock_move_item), 'UI4 Stock-status Split reclassified catalog state after Review.');
	$stock_journal = WCOS_Operation_Journal::get($stock_source, $stock_operation_id);
	wcos_p2_adapter_assert(is_array($stock_journal) && 'completed' === $stock_journal['status'], 'UI4 Stock-status journal did not complete.');
	wcos_p2_adapter_assert(WCOS_Split_Strategy_Gates::STOCK_STATUS === $stock_journal['context']['strategy_authority']['strategy'], 'UI4 Stock-status journal lost semantic strategy authority.');
} finally {
...
wcos_p2_adapter_assert(WCOS_Split_Strategy_Gates::enabled(WCOS_Split_Strategy_Gates::CATEGORY), 'Sandbox Category gate changed during UI4 E2E.');
wcos_p2_adapter_assert(WCOS_Split_Strategy_Gates::enabled(WCOS_Split_Strategy_Gates::STOCK_STATUS), 'Sandbox Stock-status gate changed during UI4 E2E.');
wcos_p2_adapter_assert(false !== has_action('wp_ajax_' . WCOS_Split_Strategy_Admin_Controller::REVIEW_ACTION), 'Sandbox Review route disappeared after UI4 E2E.');
wcos_p2_adapter_assert(false !== has_action('wp_ajax_' . WCOS_Split_Strategy_Admin_Controller::CONFIRM_ACTION), 'Sandbox Confirm route disappeared after UI4 E2E.');
wcos_p2_adapter_assert(false !== has_action('wp_ajax_' . WCOS_Split_Strategy_Admin_Controller::EXECUTE_ACTION), 'Sandbox Execute route disappeared after UI4 E2E.');

echo "p2-strategy-sandbox-ui4-ok\n";
