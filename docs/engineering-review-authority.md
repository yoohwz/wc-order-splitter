...
WOS-GOV-011 and later normal tasks use the versioned merge CI evidence, Acceptance and Human Gate records in `docs/ci-workflow-contract.md`. CI evidence remains Executor evidence or mechanical promotion; Acceptance remains ChatGPT-owned and Human Gate remains conditional human authority. Keep the PR draft through Acceptance and persist the candidate-bound Gate with `PR state: draft`; only then may ChatGPT mark it ready once. A dedicated native `pull_request: ready_for_review` workflow verifies the complete chain in its running `Required CI` job on the event merge ref, using read-only APIs. No custom check write, successful head FINAL, dispatch input, title/label/PR-body claim or Executor record substitutes for upstream roles. Finalize independently runs the read-only ruleset-aware terminal verifier: authenticate the exact completed post-Gate native check/run/app/PR association, candidate attestation, unchanged authority, every active rule and required context immediately before merge. `mergeable=true` is mandatory; `clean` needs all invariants, and `unstable` is allowed only with every enforced requirement proven successful and the remaining entries proven optional or superseded pre-Gate history. Blocked/unknown states and ambiguity remain failures. Native REST head identity and event merge SHA are separately bound, not conflated. The source-bound GOV-011 bootstrap has the same boundaries and requires new exact-head review/FINAL/Acceptance/Gate; positive recognition is proved only before merge. The failed custom-check dispatch architecture is retired.
