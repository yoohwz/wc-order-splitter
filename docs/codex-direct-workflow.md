...
Codex opens a Draft PR to `main`, runs focused local evidence, waits for protected-branch `Required CI` success with artifacts=`0`, and performs complete-diff executor self-review. The current full PR matrix remains mandatory until a separately accepted milestone adds a machine-enforced fast path without weakening branch protection.
