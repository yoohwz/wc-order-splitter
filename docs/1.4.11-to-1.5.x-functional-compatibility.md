...
| --- | --- | --- | --- |
| Split child status | `PRESERVE_FUNCTIONAL_OUTCOME` | Freeze the configured, registered source order status accepted for the Split operation and apply it to each Split child. Split and Duplicate do not share one default-status policy. | `WOS-COMPAT-002` |
