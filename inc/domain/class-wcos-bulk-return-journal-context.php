...
final class WCOS_Bulk_Return_Journal_Context {
	...
	public static function create(array $plan, $batch_id, $user_id, $raw_token, array $operation_map, $now = null) {
		...
		$anchor_id = absint($plan['canonical_child_ids'][0]);
		...
		return array(
			...
			'bulk_return_progress' => self::seal_progress(self::initial_progress(count($plan['rows'])), $authority['authority_fingerprint']),
		);
	}
	...
	public static function assert_record(array $record) {
		...
		if (self::SCHEMA_VERSION !== (int) (isset($authority['schema_version']) ? $authority['schema_version'] : 0)
	...
	public static function progress(array $current, $cursor, $status, array $results, $reason, $batch_authority_fingerprint) {
		...
		return self::seal_progress($progress, $batch_authority_fingerprint);
	}
	...
	private static function initial_progress($total) {
		return array('schema_version' => self::SCHEMA_VERSION, 'cursor' => 0, 'total' => absint($total), 'status' => 'in_progress', 'results' => array(), 'terminal_reason' => '');
	}

	private static function assert_progress(array $progress, array $authority, $batch_authority_fingerprint) {
		$total = count($authority['plan']['rows']);
		...
		if ($keys !== $expected_keys || '' === $stored_fingerprint
			...
			|| self::SCHEMA_VERSION !== (int) (isset($progress['schema_version']) ? $progress['schema_version'] : 0)
		...
		foreach ($results as $expected_ordinal => $result) {
			...
			$expected_row = isset($authority['plan']['rows'][$expected_ordinal]) ? $authority['plan']['rows'][$expected_ordinal] : array();
	...
	private static function seal_progress(array $progress, $batch_authority_fingerprint) {
		$progress['progress_fingerprint'] = self::progress_fingerprint($progress, $batch_authority_fingerprint);
		return $progress;
	}

	private static function progress_fingerprint(array $progress, $batch_authority_fingerprint) {
		...
		return WCOS_Mutation_Fingerprint::create('bulk_return_progress_v1', absint(isset($progress['total']) ? $progress['total'] : 0), array(
	...
	private static function authority_fingerprint(array $authority) {
		...
		return WCOS_Mutation_Fingerprint::create('bulk_return_coordinator_v1', absint(isset($authority['anchor_child_id']) ? $authority['anchor_child_id'] : 0), self::canonicalize($authority));
	}
